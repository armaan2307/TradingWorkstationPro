...
def style_dataframe(df):
    """Formats numbers cleanly and highlights active trades based on live P&L and daily movement."""
    if df.empty:
        return df

    df_styled = df.copy()

    # 1. Fallback for Intraday Hub 1D Change if missing
    ...
    # 2. Live P&L and Daily % Change Calculation for Swing / Long-Term
            ...
            for idx in df_styled[active_mask].index:
                try:
                    ...
                    if len(t_data) >= 2:
                        ...
                        # Live P&L relative to entry
                        pnl_pct = ((curr_price - entry_price) / entry_price) * 100
                        
                        # If trade was created today and entry == curr_price, use today's 1D change
                        if abs(pnl_pct) < 0.01:
    ...
    # 3. Enhanced Row Color Helper (Highlights positive >= 0 in soft green, negative < 0 in soft red)
    def color_rows(row):
        ...
        if pd.notna(val):
            if val > 0:
                ...
                return ['background-color: #d4edda; color: #155724; font-weight: bold'] * len(row) # Highlight flat/0% as green
        return [''] * len(row)
